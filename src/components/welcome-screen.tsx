...
import { ScrollOverview } from '@/components/scroll-overview';
...
export function WelcomeScreen({
  ...
  return (
    <div data-scroll-root className="flex-1 flex flex-col items-center justify-start overflow-y-auto custom-scrollbar relative">
      {/* Apple-style scroll-driven hero overview */}
      <ScrollOverview language={language} onGetStarted={onFocusChat} />

      {/* Existing welcome content (padded from here on) */}
      <div className="w-full px-4 sm:px-6 py-6 sm:py-8 flex flex-col items-center">
      {/* Animated gradient background blobs — sized down on mobile to save GPU */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div
          className="absolute -top-32 -left-32 w-[260px] h-[260px] sm:w-[500px] sm:h-[500px] rounded-full opacity-20 dark:opacity-10"
          style={{
        ...
        />
        <div
          className="absolute -bottom-24 -right-24 w-[220px] h-[220px] sm:w-[400px] sm:h-[400px] rounded-full opacity-15 dark:opacity-[0.08]"
          style={{
        ...
        />
        <div
          className="hidden sm:block absolute top-1/3 right-0 w-[300px] h-[300px] rounded-full opacity-10 dark:opacity-[0.05]"
          style={{
      ...
      <div className="max-w-2xl w-full mx-auto relative z-10">
        ...
        >
          ...
          <div className="-mx-4 px-4 sm:mx-0 sm:px-0 flex sm:grid sm:grid-cols-3 lg:grid-cols-6 gap-2 overflow-x-auto sm:overflow-visible snap-x-mobile scrollbar-hide pb-1 sm:pb-0">
            {featuredProtocolIds.map((id) => {
              ...
              return (
                <button
                  ...
                  className={`group relative shrink-0 w-[44%] sm:w-auto rounded-xl border ${colors.border} ${colors.bg} p-3 text-left transition-all duration-200 hover:shadow-lg hover:shadow-black/[0.04] dark:hover:shadow-black/20 hover:-translate-y-1 hover:scale-[1.03] active:translate-y-0 active:scale-[0.97] overflow-hidden touch-manipulation`}
                >
        ...
        {/* Quick action bar — keyboard shortcuts, hidden on phone */}
        <div
          className={`hidden sm:flex flex-wrap items-center justify-center gap-x-2 gap-y-1.5 mt-6 transition-all duration-700 delay-400 ${
            mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          }`}
        >
